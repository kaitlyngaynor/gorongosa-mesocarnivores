...
The next chunk seem like factors influencing the co-occurrence of pairs of species.
* F12 - 2 columns, have a value of "1" for all 3 rows in which species 1 and 2 are present
* F13 - 2 columns, have a value of "1" for all 3 rows in which species 1 and 3 are present
* F14 - 2 columns, have a value of "1" for all 3 rows in which species 1 and 4 are present
* F23 - 2 columns, have a value of "1" for all 3 rows in which species 2 and 3 are present
* F24 - 2 columns, have a value of "1" for all 3 rows in which species 2 and 4 are present
* F34 - 2 columns, have a value of "1" for all 3 rows in which species 3 and 4 are present
...
for(i in 1:nrow(bobcat)){ ## for every row (camera)
  for(j in 1:15){ # Not sure why this is 15?????
    X.array[i, j, dm[j, ] == 1] <-
...
Need to dig into this a little bit more to fully understand what's up.
...
The descriptions of some of these values are in the model3.stan file, and others are in to be found in the code above.
...
data <- list(
  ...
  dd = scale(dd)[, 1], # vector of detections?
