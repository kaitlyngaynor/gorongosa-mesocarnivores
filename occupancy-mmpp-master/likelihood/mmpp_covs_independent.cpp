...
double mmpp_covs_i(arma::vec params, arma::mat pind, 
    arma::mat X_f1, arma::mat X_f2, 
    arma::mat X_lam1, arma::mat X_lam2,
  ...
  vec mu1 = exp(params.subvec(pind(2,0), pind(2,1)));
  vec mu2 = exp(params.subvec(pind(3,0), pind(3,1)));

  vec lambda1 = exp(X_lam1 * params.subvec(pind(4,0), pind(4,1)));
  vec lambda2 = exp(X_lam2 * params.subvec(pind(5,0), pind(5,1)));
  ...
  for (unsigned i=0; i<N; i++){
    ...
    for (unsigned j=0; j<lambda1_i.size(); j++){
      ...
      ll2 += splt_lik(yd2_j, lambda2_i(j), zet2, mu2, e);
    }

    double l1 = exp(ll1);
    double l2 = exp(ll2);

    //Set for detection independence, need to change for other models
    loglik += y1_i(i) * y2_i(i) * 
		log(psi(0) * l1 * l2) +
            y1_i(i) * (1-y2_i(i)) * 
		log(psi(0) * l1 * l2 + 
		    psi(1) * l1) +
