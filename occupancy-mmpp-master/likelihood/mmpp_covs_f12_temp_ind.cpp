...
double mmpp_covs_temp_i(arma::vec params, arma::mat pind, arma::mat X_f1, arma::mat X_f2,
    arma::mat X_f12, 
  ...
  for (unsigned i=0; i<N; i++){
    ...
    double l1 = exp(ll1);//sp1
    double l2 = exp(ll2);//sp2

    //Set for detection independence, need to change for other models
    loglik += y1_i(i) * y2_i(i) * log(psi(0) * l1 * l2) +
	      y1_i(i) * (1-y2_i(i)) * log(psi(0) * l1 * l2 + psi(1) * l1) +
              (1-y1_i(i)) * y2_i(i) * log(psi(0) * l1 * l2 + psi(2) * l2) +
              (1-y1_i(i)) * (1-y2_i(i)) *
                    log(psi(0) * l1 * l2 + psi(1) * l1 + psi(2) * l2 + psi(3));
  }
