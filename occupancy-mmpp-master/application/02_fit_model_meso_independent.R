...
#this is mm2
X_f1_GNP2 <- model.matrix(~1, site_covs_GNP) #civet
X_f2_GNP2 <- model.matrix(~1, site_covs_GNP) #genet
#X_f12_GNP <- model.matrix(~lion_latedry_scaled, site_covs_GNP) #no spatial interaction
...
#species 1 (regardless of species 2)
X_lam1_GNP2 <- model.matrix(~cover.ground, obs_covs_GNP)

#NOT USED species 1, species 2 absent
#X_lam2_GNP <- model.matrix(~f1c_GNP + f2c_GNP + f1s_GNP + f2s_GNP + detect.obscured + cover.ground, obs_covs_GNP)

#species 2
X_lam2_GNP2 <- model.matrix(~cover.ground, obs_covs_GNP)

# Save model matrices for use elsewhere
save(X_f1_GNP2, X_f2_GNP2, X_lam1_GNP2, X_lam2_GNP2, file='model_matrices_GNP2.Rdata')

# Indicator matrix to divide up single vector of parameters into
# subvectors for each parameter to estimate
#KLG: not sure I follow why the different values are used
pind_GNP2 <- matrix(NA, nrow=6, ncol=2) #KLG: makes an empty matrix with 6 rows and 2 columns
#KLG: each row is filled in individually
pind_GNP2[1,] <- c(0, 0+ncol(X_f1_GNP2)-1)                #f1, KLG: fills in first row
pind_GNP2[2,] <- c(pind_GNP2[1,2]+1, pind_GNP2[1,2]+1+ncol(X_f2_GNP2)-1)    #f2
#pind_GNP[3,] <- c(pind_GNP[2,2]+1, pind_GNP[2,2]+1+ncol(X_f12_GNP)-1)   #f12
pind_GNP2[3,] <- c(pind_GNP2[2,2]+1, pind_GNP2[2,2]+2)                 #mu (species 1)
pind_GNP2[4,] <- c(pind_GNP2[3,2]+1, pind_GNP2[3,2]+2)                 #mu (species 2)
pind_GNP2[5,] <- c(pind_GNP2[4,2]+1, pind_GNP2[4,2]+1+ncol(X_lam1_GNP2)-1)  #lambda sp1
pind_GNP2[6,] <- c(pind_GNP2[5,2]+1, pind_GNP2[5,2]+1+ncol(X_lam2_GNP2)-1)  #lambda sp2
#pind_GNP[8,] <- c(pind_GNP[7,2]+1, pind_GNP[7,2]+1+ncol(X_lam3_GNP)-1)  #lambda sp2 (NOT)
...
# KLG: this is used to feed into the next optimization thing, it generates start values
#KLG: rep() replicates the values in x
starts_GNP2 <- optim(rep(0,max(pind_GNP2)+1), mmpp_covs_i, method = 'SANN',
                     control = list(maxit=400, trace=1, REPORT =5),
                     pind=pind_GNP2, X_f1=X_f1_GNP2, X_f2=X_f2_GNP2, X_lam1=X_lam1_GNP2,
                     X_lam2=X_lam2_GNP2, yd1=yd1_GNP, yd2=yd2_GNP, lidx_i=lidx_i_GNP,
                     yd1_st_idx=yd1_st_idx_GNP, yd1_en_idx=yd1_en_idx_GNP, yd2_st_idx=yd2_st_idx_GNP,
                     yd2_en_idx=yd2_en_idx_GNP, y1_i=y1_i_GNP, y2_i=y2_i_GNP, threads=2)
# Final SANN NLL value should be 30050.156712
# KLG: ~12-15 min to run above code
...
# Do optimization and calculate hessian
fit_GNP2 <- optim(starts_GNP2$par, mmpp_covs_i, method = 'L-BFGS-B', hessian=TRUE,
                  control = list(trace = 1, REPORT = 5, maxit=400),
                  pind=pind_GNP2, X_f1=X_f1_GNP2, X_f2=X_f2_GNP2, X_lam1=X_lam1_GNP2,
                  X_lam2=X_lam2_GNP2, yd1=yd1_GNP, yd2=yd2_GNP, lidx_i=lidx_i_GNP,
                  yd1_st_idx=yd1_st_idx_GNP, yd1_en_idx=yd1_en_idx_GNP, yd2_st_idx=yd2_st_idx_GNP,
...
#KLG: saveRDS saves an R object for it to be called later (it serializes an R object into a 
#KLG: format that can be called later), but it forgets the original name of the object
saveRDS(fit_GNP2, "fit_covs3_GNP2.Rds")

est_GNP2 <- fit_GNP2$par 
names(est_GNP2) <- c(paste0("f1_",colnames(X_f1_GNP2)), paste0("f2_",colnames(X_f2_GNP2)),
                     "log_mu1[1]","log_mu1[2]","log_mu2[1]","log_mu2[2]",
                     paste0("loglam1_",colnames(X_lam1_GNP2)),
                     paste0("loglam2_",colnames(X_lam2_GNP2)))
se_GNP2 <- sqrt(diag(solve(fit_GNP2$hessian))) 

results_GNP2 <- data.frame(est = round(est_GNP2, 3), se=round(se_GNP2,3))
#results_GNP <- data.frame(est_GNP = round(est_GNP, 3))
results_GNP2$lower <- results_GNP2$est - 1.96*results_GNP2$se
results_GNP2$upper <- results_GNP2$est + 1.96*results_GNP2$se
results_GNP2

saveRDS(results_GNP2, 'results_GNP2.Rds')

#AIC
(2*length(fit_GNP2$par))-(2*-fit_GNP2$value)
