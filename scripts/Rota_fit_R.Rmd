...
---
Collecting and annotating scripts for running occuMulti, an R function that fits Rota's model. (not going to try to dig completely into every step of this, in case you have a reason not to use it. more looking for overall understanding/a way to take notes so we'd be closer to ready to use it if you think it would be a good option)
...
#so this would be how we want the detection covariates to be considered? not sure how well I understand right now
...
#natural parameters = f1, f2, f12, etc? the formulas that include which factors we believe will affect occupancy? 
...
#is this the first replicate survey at each site? that's something the stan code uses
...
#occ_covs has 1000 rows, 10 columns; don't actually understand what they represent though
...
#this creates a spreadsheet with columns for sp 1, sp 2, sp 3, sp 1:sp 2, sp 1:sp 3, sp 2:sp 3, sp 1:sp 2:sp 3, which seems promising?
...
# fake y data
##is this the Y1, Y2, etc. we've been working with?
y <- list()
for (i in 1:nspecies){
y[[i]] <- matrix(NA,N,J)
for (j in 1:N){
...
##wait...are the detection covariates the observation covariates here?
...
#no idea. what type of covariate would vary within a site? I think this would be NULL for our data. Unless these are the detection covariates? some hint of that, maybe, above
...
#is the below code just creating a matrix for use?
