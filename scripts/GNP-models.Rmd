...
#trying to replicate the values to take care of the error I am getting
detect.obscured <- det_covs[,1] #first column
#detect.obscured2 <- cbind(detect.obscured, detect.obscured)
n <- 122 #total number of days
detect.obscured.n <- do.call("cbind", replicate(n, detect.obscured, simplify = FALSE)) #repeat columns for each day

#detect.obscured.n <- matrix(detect.obscured, length(detect.obscured), 122) #repeat for each day

cover.ground <- det_covs[,2] #second column
cover.ground.n <- do.call("cbind", replicate(n, cover.ground, simplify = FALSE)) #repeat columns for each day

#cover.ground.n <- matrix(cover.ground, length(cover.ground), 122) #repeat for each day
