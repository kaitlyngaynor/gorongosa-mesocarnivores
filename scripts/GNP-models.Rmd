...
occFormulas <- c('~urema_dist','~tree_hansen','~termite.large.count.100m','~lion_camera','~1','~1','~1','~1','~1','~1')
...
fit <- occuMulti(detFormulas, stateformulas = occFormulas, data, maxOrder = 2)
...
# but how do we get these into data frames that we can then plot?? having trouble with S4 object conversion... as.data.frame, as_tibble, tidy, as.matrix don't work
...
head(predict(fit,'state',species='genet'))
head(predict(fit,'det',species='civet'))
```
